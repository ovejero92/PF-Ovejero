...
export const authReducer = createReducer(
  ...
  on(authActions.loginSuccess, (state, { user }) => ({ ...state, authUser: user, loading: false, error: null })),
  on(authActions.loginFailure, (state, { error }) => ({ ...state, authUser: null, loading: false, error })),

  on(authActions.register, (state) => ({ ...state, loading: true, error: null })),
  on(authActions.registerSuccess, (state, { user }) => ({ ...state, authUser: user, loading: false, error: null })),
  on(authActions.registerFailure, (state, { error }) => ({ ...state, loading: false, error })),

  on(authActions.verifyTokenSuccess, (state, { user }) => ({ ...state, authUser: user })),
  on(authActions.verifyTokenFailure, (state) => ({ ...state, authUser: null })),
